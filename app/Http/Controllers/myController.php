...
class myController extends Controller
{
    ...
    public function giayTheoThuongHieu($slug){
     ...
     public function giay(){
        ...
        return view('pages.product',compact('all_product'));
    }
    ...
    public function giayChiTiet($slug){
        ...
        ->limit(4) 
        ->get();
         $comment = tbl_comment::where('product_id',$product_detail->id)->orderBy('created_at','desc')->get(); 
        return view('pages.product_detail',compact('product_detail','related_products','comment'));
    ...
    function timkiem(Request $request){
        ...
        $timkiem = \DB::table('tbl_product')
        ->where('product_name','like','%'.$tukhoa.'%')
        ->paginate($perpage)->withQueryString();   
        return view('pages.search',compact('timkiem','tukhoa')); 
    }
}